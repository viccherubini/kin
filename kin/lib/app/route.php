...
class route {
	...
	private function check_method_is_valid($method) {
		...
		if (!in_array($method, array(self::get, self::post, self::put, self::delete))) {
			throw new unrecoverable("The route can not be properly constructed. The method {$method} is not one of GET, POST, PUT, or DELETE.");
		}
		return($this);
	}
	...
	private function check_route_is_nonempty($route) {
		if (0 === strlen($route)) {
			throw new unrecoverable("The route can not be empty.");
		}
		return($this);
	}
	...
	private function check_route_starts_with_slash($route) {
		if ("/" !== $route[0]) {
			throw new unrecoverable("The route must begin with a forward slash: /.");
		}
		return($this);
	}
	...
	private function check_route_matches_regex($route) {
		if (0 === preg_match('#^/([a-z]+)([a-z0-9_\-/%\.\*]*)$#i', $route)) {
			throw new unrecoverable("The route {$route} is not valid.");
		}
		return($this);
	}
